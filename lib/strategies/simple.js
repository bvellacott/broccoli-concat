...
SimpleConcat.prototype = merge(SimpleConcat.prototype, {
  ...
  write: function() {
    ...
    this._internal.forEach(function(entry) {
      ...
      entry.content = that.transform.call(context, entry.file, entry.content)

      entry.header = []
        .concat(
          typeof that.header === 'function' ? 
            that.header.call(context, entry.file, entry.content) : 
            (that.header || ''), 
      ...
      entry.footer = []
        .concat(
          ...
          typeof that.footer === 'function' ? 
            that.footer.call(context, entry.file, entry.content) : 
